      ...
      {backups.length === 0 ? (
        ...
        <Stack direction="row" spacing={2} flexWrap="wrap">
          {backups.map(b => (
            <Card
              key={b.id}
              sx={{ width: 240, height: 150, flex: "0 0 240px", display: "flex", alignItems: "stretch", position: "relative" }}
            >
              <IconButton
                size="small"
              ...
              <CardContent sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 0.5, justifyContent: "space-between", height: "100%", p: 2 }}>
                <Box display="flex" alignItems="center" gap={1}>
                  <Box fontSize={28}>{getSourceIcon(b.source.backupModuleId)}</Box>
                  <Typography variant="caption">→</Typography>
                  <Box fontSize={28}>{getSourceIcon(b.storage.backupModuleId)}</Box>
                </Box>
